import collections,copy,glob,io,lzma,math,os,traceback
...
class ValidationData:
        def __init__(self,h,device):
                ...
                self.val_tokens=load_validation_tokens(h.val_files,h.eval_seq_len,h.max_val_tokens);self.base_bytes_lut,self.has_leading_space_lut,self.is_boundary_token_lut=build_sentencepiece_luts(self.sp,h.vocab_size,device)
...
def load_data_shard(file):
        ...
        return torch.from_numpy(np.fromfile(file,dtype='<u2',count=-1,offset=header_bytes).astype(np.int32))
...
class RMSNorm(nn.Module):
        def __init__(self,dim,eps=1e-06,scale=True):super().__init__();self.eps=eps;self.weight=nn.Parameter(torch.ones(dim))if scale else None
        def _norm(self,x):return x*torch.rsqrt(x.pow(2).mean(-1,keepdim=True)+self.eps)
        def forward(self,x):output=self._norm(x.float()).type_as(x);return output*self.weight if self.weight is not None else output
class CastedLinear(nn.Linear):
        def __init__(self,in_features,out_features,bias=True):super().__init__(in_features,out_features,bias)
        def cast(self,dtype):self.weight.data=self.weight.data.to(dtype=dtype)
        def forward(self,x):return F.linear(x.to(self.weight.dtype),self.weight,self.bias)
class LoRALinear(nn.Module):
    def __init__(self, linear_layer, rank=8, alpha=1.0):
        ...
        self.lora_A = nn.Parameter(torch.zeros(self.linear.weight.size(1), rank, device=dev))
        self.lora_B = nn.Parameter(torch.zeros(rank, self.linear.weight.size(0), device=dev))
        nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
        nn.init.zeros_(self.lora_B)

    def forward(self, x):
        lora_out = (x @ self.lora_A @ self.lora_B) * self.alpha
        return self.linear(x) + lora_out

class CastedLoRALinear(LoRALinear):
    def forward(self, x):
        x_dtype = x.dtype
        linear_out = self.linear(x)
        x_cast = x.to(self.lora_A.dtype)
        lora_out = (x_cast @ self.lora_A @ self.lora_B) * self.alpha
        return linear_out + lora_out.to(x_dtype)

    def cast(self, dtype):
        if hasattr(self.linear, 'cast'):
            self.linear.cast(dtype)
        self.lora_A.data = self.lora_A.data.to(dtype)
        self.lora_B.data = self.lora_B.data.to(dtype)
def xavier_normal_matmuls(m):
        if isinstance(m,nn.Linear):
                nn.init.xavier_normal_(m.weight)
                if m.bias is not None:nn.init.zeros_(m.bias)
class LeakySquared(nn.Module):
        def __init__(self,alpha):super().__init__();self.alpha=alpha
        def forward(self,x):return F.leaky_relu(x,self.alpha)**2
class CausalSelfAttention(nn.Module):
        def __init__(self,h,layer_num):
                super().__init__();self.h=h;self.num_heads=h.num_heads;self.num_kv_heads=h.num_kv_heads;self.head_dim=h.model_dim//h.num_heads;self.q_proj=CastedLinear(h.model_dim,h.model_dim);self.k_proj=CastedLinear(h.model_dim,h.num_kv_heads*self.head_dim);self.v_proj=CastedLinear(h.model_dim,h.num_kv_heads*self.head_dim);self.o_proj=CastedLinear(h.model_dim,h.model_dim)
                if h.qk_gain_init is not None and h.qk_gain_init>0:self.q_gain=nn.Parameter(torch.full((h.num_heads,1,1),h.qk_gain_init))
                else:self.q_gain=None
                self.rope_cache=None;self.layer_num=layer_num
        def get_rope_cache(self,x):
                if self.rope_cache is not None and self.rope_cache.size(0)>=x.size(1):return self.rope_cache
                self.rope_cache=self.build_rope_cache(x);return self.rope_cache
        def build_rope_cache(self,x):
                h=self.h;L=x.size(1);min_custom_base=h.rope_base*h.rope_train_seq_len/L if L>h.rope_train_seq_len else h.rope_base;theta=1./(min_custom_base**(torch.arange(0,h.rope_dims,2).float()/h.rope_dims)).to(x.device);t=torch.arange(L,device=x.device,dtype=torch.float32);freqs=torch.einsum('i,j->ij',t,theta);return freqs
        def apply_rotary_emb(self,x,rope_cache):
                x=x.transpose(1,2);target_len=x.size(2);rope_cache=rope_cache[:target_len].to(dtype=x.dtype).unsqueeze(0).unsqueeze(0);x_rope,x_pass=x[...,:self.h.rope_dims],x[...,self.h.rope_dims:];x_rope_half=x_rope.size(-1)//2;x_r1,x_r2=x_rope.split(x_rope_half,dim=-1);x_out_r1=x_r1*rope_cache.cos()-x_r2*rope_cache.sin();x_out_r2=x_r1*rope_cache.sin()+x_r2*rope_cache.cos();x_rope=torch.cat((x_out_r1,x_out_r2),dim=-1);return torch.cat((x_rope,x_pass),dim=-1).transpose(1,2)
        def forward(self,x):
                B,L,D=x.shape;is_xsa_layer=self.h.xsa_last_n is not None and self.layer_num>=self.h.num_layers-self.h.xsa_last_n;q=self.q_proj(x);k=self.k_proj(x);v=self.v_proj(x)
                if self.q_gain is not None:q=q.unflatten(-1,(self.num_heads,self.head_dim)).transpose(1,2);q*=self.q_gain.to(q.dtype);q=q.transpose(1,2).flatten(-2)
                rope_cache=self.get_rope_cache(x)
                # apply_rotary_emb: input [B,L,H,D], output [B,L,H,D]; then transpose to [B,H,L,D]
                q=self.apply_rotary_emb(q.unflatten(-1,(self.num_heads,self.head_dim)),rope_cache).transpose(1,2)
...
class Block(nn.Module):
        def __init__(self,h,layer_num):
                super().__init__();self.h=h;self.layer_num=layer_num;self.norm1=RMSNorm(h.model_dim,scale=h.ln_scale);self.attn=CausalSelfAttention(h,layer_num);self.norm2=RMSNorm(h.model_dim,scale=h.ln_scale);self.mlp=MLP(h.model_dim,h.mlp_mult);self.parallel_res=h.parallel_residual_start is not None and layer_num>=h.parallel_residual_start
        def forward(self,x,skip_x=None):
                if self.parallel_res:
                        resid=self.norm1(x);attn_out=self.attn(resid);mlp_out=self.mlp(resid);return x+attn_out+mlp_out,skip_x
                else:
                        resid=x+self.attn(self.norm1(x))
                        if skip_x is not None:resid=resid+skip_x
                        return resid+self.mlp(self.norm2(resid)),skip_x
class GPT(nn.Module):
        def __init__(self,h):
                super().__init__();self.h=h;self.tok_emb=nn.Embedding(h.vocab_size,h.embedding_dim);nn.init.normal_(self.tok_emb.weight,std=h.tied_embed_init_std);self.blocks=nn.ModuleList([Block(h,i)for i in range(h.num_layers)])
                if h.model_dim!=h.embedding_dim:self.head_proj=CastedLinear(h.embedding_dim,h.model_dim)
                else:self.head_proj=None
                self.final_norm=RMSNorm(h.model_dim,scale=h.ln_scale)
                if h.tie_embeddings:self.lm_head=None;self.tok_emb.weight.data=self.tok_emb.weight.data.float()
                else:self.lm_head=CastedLinear(h.model_dim,h.vocab_size,bias=False)
                if h.skip_gates_enabled:self.skip_gates=nn.Parameter(torch.zeros(h.num_layers))
                else:self.skip_gates=None
                self.looping_active=False
                if h.num_loops>0:
                        encoder_len=h.loop_end-h.loop_start;self.encoder_indices=list(range(h.loop_start,h.loop_end));self.decoder_indices=list(range(h.loop_end,h.loop_end+encoder_len));log(f'loop:init encoder={self.encoder_indices} decoder={self.decoder_indices}')
                        if h.skip_gates_enabled:self.skip_weights=nn.Parameter(torch.zeros(h.num_layers,h.num_loops-1))
                        else:self.skip_weights=torch.zeros(h.num_layers,h.num_loops-1)
                else:self.encoder_indices=self.decoder_indices=[]
        def forward_logits(self,idx):
                x=self.tok_emb(idx);x=self.head_proj(x)if self.head_proj else x;skip_states=[None]*self.h.num_layers
                for i,block in enumerate(self.blocks):
                        if self.looping_active and i in self.encoder_indices:
                                loop_idx=self.encoder_indices.index(i)
                                for l in range(self.h.num_loops):
                                        skip_x=skip_states[i]
                                        if skip_x is not None and l>0 and self.h.skip_gates_enabled:skip_x=skip_x*torch.sigmoid(self.skip_weights[i,l-1])
                                        x,skip_states[i]=block(x,skip_x)
                        elif self.looping_active and i in self.decoder_indices:
                                loop_idx=self.decoder_indices.index(i);enc_i=self.encoder_indices[loop_idx];skip_x=skip_states[enc_i]
                                if skip_x is not None and self.h.skip_gates_enabled:skip_x=skip_x*torch.sigmoid(self.skip_gates[enc_i])
                                x,_=block(x,skip_x=skip_x)
                        else:x,_=block(x)
                x=self.final_norm(x)
                if self.lm_head is not None:logits=self.lm_head(x)
                else:logits=F.linear(x,self.tok_emb.weight.to(x.dtype))
                if self.h.logit_softcap>0:logits=torch.tanh(logits/self.h.logit_softcap)*self.h.logit_softcap
                return logits
        def forward(self,idx,targets):
                logits=self.forward_logits(idx);loss=F.cross_entropy(logits.view(-1,logits.size(-1)),targets.view(-1));return loss
def classify_param(name):
        if'tok_emb'in name:return'emb'
        if'norm'in name or'bias'in name:return'norm'
        if'q_gain'in name or'skip'in name:return'skip'
        if'head'in name:return'out'
        if'attn'in name:return'attn'
        if'mlp'in name:return'mlp'
        return'other'
@torch.no_grad()
def zeropower_via_newtonschulz5(X,steps=5):
        norm_orig=torch.linalg.matrix_norm(X);Y=X/norm_orig
        for _ in range(steps):Y=Y@(1.5*torch.eye(Y.shape[1],device=Y.device,dtype=Y.dtype)-.5*(Y.T@Y))
        return Y*norm_orig
class Muon(torch.optim.Optimizer):
        def __init__(self,params,lr,momentum,backend_steps=5,nesterov=True,weight_decay=0,row_normalize=True):
                if lr<0:raise ValueError(f"Invalid learning rate: {lr}")
                defaults=dict(lr=lr,momentum=momentum,backend_steps=backend_steps,nesterov=nesterov,weight_decay=weight_decay,row_normalize=row_normalize);super().__init__(params,defaults)
        def step(self,closure=None):
                loss=None
                if closure is not None:loss=closure()
                if not self.param_groups:return loss
                world_size=dist.get_world_size()if dist.is_available()and dist.is_initialized()else 1;rank=dist.get_rank()if dist.is_available()and dist.is_initialized()else 0
                for group in self.param_groups:
                        ...
                        for(i,p)in enumerate(params):
                                ...
                                curr+=p.numel()
                        if world_size > 1:dist.all_reduce(updates_flat,op=dist.ReduceOp.SUM)
                        wd=group.get('weight_decay',.0);curr=0
                        for p in params:
                                if wd>.0:p.data.mul_(1.-lr*wd)
                                g=updates_flat[curr:curr+p.numel()].view_as(p).to(dtype=p.dtype);p.data.add_(g,alpha=-lr);curr+=p.numel()
                return loss
...
class Optimizers:
        def __init__(self,h,base_model):
                ...
                if hasattr(base_model, 'skip_weights') and base_model.skip_weights is not None and base_model.skip_weights.numel()>0:scalar_params.append(base_model.skip_weights)
...
def gptq_mixed_quantize(state_dict,hessians,h):
        ...
        for(name,cat)in meta.items():short=re.sub(r'\.\d+$','',re.sub(r'blocks\.\d+','blocks',name));categories[cat].add(short)
        log('Quantized weights:')
        for cat in sorted(categories):log("  {}: {}".format(cat,", ".join(sorted(categories[cat]))))
        return result,meta
...
def eval_val(h,device,val_data,model):
        ...
        num_batches=(seq_end-seq_start+local_batch_seqs-1)//local_batch_seqs;log(f"eval_val:start seqs={seq_end-seq_start} batches={num_batches}")
        with torch.inference_mode():
                for bi,batch_seq_start in enumerate(range(seq_start,seq_end,local_batch_seqs)):
                        if bi>0 and bi%200==0:log(f"eval_val:progress {bi}/{num_batches}")
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
        ...
        log(f"LoRA ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} lora_params={len(lora_params)}");
        ...
        optimizer=torch.optim.SGD(lora_params,lr=h.ttt_lr,momentum=h.ttt_momentum)
                        ...
                        if chunk_seqs>0:
                                ...
                                for _ep in range(h.ttt_epochs):
                                        for bs in range(0,my_chunk_seqs,batch_seqs):
                                                ...
                                                torch.nn.utils.clip_grad_norm_(lora_params,1.);optimizer.step()
        if dist.is_available()and dist.is_initialized():dist.all_reduce(loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(token_count,op=dist.ReduceOp.SUM);dist.all_reduce(byte_count,op=dist.ReduceOp.SUM)
        ...
        while True:
                ...
                if should_validate:
                        try:
                                ...
                                val_loss,val_bpb=eval_val(h,device,val_data,model);log(f"{step}/{h.iterations} val_loss: {val_loss:.4f} val_bpb: {val_bpb:.4f}")
                                torch.cuda.synchronize();t0=time.perf_counter()
                        except Exception as _eval_err:
                                log(f"eval_val CRASHED at step {step}: {_eval_err}\n{traceback.format_exc()}");raise
                if last_step:
...
def train_and_eval(h,device):
        random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log("train_shards: {}".format(len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))));log(f"val_tokens: {val_data.val_tokens.numel()-1}");base_model,compiled_model=train_model(h,device,val_data);torch._dynamo.reset();timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model);serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
...
def main():
        ...
        if bool(int(os.environ.get('NO_COMPILE','0'))):torch._dynamo.config.suppress_errors=True;torch.compile=lambda m,**kw:m
        from torch.backends.cuda import enable_cudnn_sdp,enable_flash_sdp,enable_math_sdp,enable_mem_efficient_sdp;enable_cudnn_sdp(False);enable_flash_sdp(True);enable_mem_efficient_sdp(True);enable_math_sdp(True);torch._dynamo.config.optimize_ddp=False;h=Hyperparameters();set_logging_hparams(h)
        if h.is_main_process:
